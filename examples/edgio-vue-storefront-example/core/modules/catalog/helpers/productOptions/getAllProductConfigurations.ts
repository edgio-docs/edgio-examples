 ...
 */
export default function getAllProductConfigurations ({
  configurableOptions,
  configuration
}): ProductOptions {
  ...
  product_option.extension_attributes.configurable_item_options = Object.keys(configuration)
    ...
    .filter(
      configOption =>
        configOption && configOption.attribute_code && configOption.attribute_code !== 'price'
    )
    .map(configOption => ({
      configOption,
      productOption: configurableOptions.find(
        productConfigOption => productConfigOption.attribute_code === configOption.attribute_code
      )
    }))
