...
export const buildProductSearchVars = ({ categoryCode ='' , pageSize =5 , filters ={} , startIndex =0 , sort ='' , search ='' ,  })=>{
    let facetTemplate = '';
    let filter = '';
    let sortBy;
    if (categoryCode) {
    ...
    const facetFilterList = Object.keys(filters).filter((k)=>filters[k].length
    ).reduce((accum, k)=>{
        return [
            ...accum,
