...
describe('product/setCurrent action', () => {
  ...
  it('should commit product data and configuration', async () => {
    ;(getProductConfigurationOptions as jest.Mock).mockImplementation(() => ({
      color: [{ attribute_code: 'color', id: '42', label: 'Green' }],
    }))
    const wrapper = (actions: any) => actions.setCurrent(contextMock, product)
    await wrapper(productActions)
    expect(contextMock.commit).toHaveBeenNthCalledWith(
      1,
      mutationTypes.PRODUCT_SET_CURRENT_OPTIONS,
      { color: [{ attribute_code: 'color', id: '42', label: 'Green' }] }
    )
    expect(contextMock.commit).toHaveBeenNthCalledWith(
      2,
      mutationTypes.PRODUCT_SET_CURRENT_CONFIGURATION,
      { color: 42 }
    )
    expect(contextMock.commit).toHaveBeenNthCalledWith(3, mutationTypes.PRODUCT_SET_CURRENT, {
      sku: 'sku',
    })
  })
  it('should call setProductGallery if mergeConfigurableChildren is set false', async () => {
    const wrapper = (actions: any) => actions.setCurrent(contextMock, product)
    await wrapper(productActions)
    expect(contextMock.dispatch).toHaveBeenNthCalledWith(1, 'setProductGallery', {
      product: { sku: 'sku' },
    })
  })
