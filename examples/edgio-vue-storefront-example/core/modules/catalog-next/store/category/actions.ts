...
const actions: ActionTree<CategoryState, RootState> = {
  async loadCategoryProducts (
    ...
    const { items, perPage, start, total, aggregations, attributeMetadata } = await dispatch(
      'product/findProducts',
      {
        query: filterQr,
        sort:
          searchQuery.sort || `${products.defaultSortBy.attribute}:${products.defaultSortBy.order}`,
  ...
  async cacheProducts ({ commit, getters, dispatch, rootState }, { route } = {}) {
    ...
    const cachedProductsResponse = await dispatch(
      'product/findProducts',
      {
        query: filterQr,
    ...
    if (products.filterUnavailableVariants) {
  ...
  async loadCategories (
    ...
    const searchingByIds = !(
      !categorySearchOptions ||
      !categorySearchOptions.filters ||
      !categorySearchOptions.filters.id
    )
    const searchedIds: string[] = searchingByIds
      ? [...categorySearchOptions.filters.id].map(String)
      : []
    const loadedCategories: Category[] = []
    if (searchingByIds && !categorySearchOptions.reloadAll) {
      // removing from search query already loaded categories, they are added to returned results
      ...
      categorySearchOptions.filters.id = searchedIds.filter(
        categoryId =>
          !getters.getCategoriesMap[categoryId] &&
          !getters.getNotFoundCategoryIds.includes(categoryId)
      )
    }
    if (!searchingByIds || categorySearchOptions.filters.id.length) {
      categorySearchOptions.filters = Object.assign(
        cloneDeep(config.entities.category.filterFields),
        categorySearchOptions.filters ? cloneDeep(categorySearchOptions.filters) : {}
      )
      ...
      const notFoundCategories = searchedIds.filter(
        categoryId =>
          !categories.some(cat => cat.id === parseInt(categoryId) || cat.id === categoryId)
      )
  ...
  async loadAvailableFiltersFrom (
    ...
    if (categoryMappedFilters && filtersKeys.length) {
      resultFilters = Object.assign(
        ...
        cloneDeep(omit(aggregationFilters, filtersKeys))
      )
    }
  ...
  async loadCategoryBreadcrumbs (
    ...
    if (!category) return
    const categoryHierarchyIds = category.parent_ids
      ? [...category.parent_ids, category.id]
      : _prepareCategoryPathIds(category) // getters.getCategoriesHierarchyMap.find(categoryMapping => categoryMapping.includes(category.id))
    const categoryFilters = Object.assign(
      { id: categoryHierarchyIds },
    ...
    await dispatch(
      'breadcrumbs/set',
      { current: currentRouteName, routes: parseCategoryPath(sorted) },
      { root: true }
    )
    return sorted
  },
