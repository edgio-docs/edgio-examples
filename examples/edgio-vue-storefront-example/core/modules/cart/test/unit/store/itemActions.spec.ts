...
describe('Cart itemActions', () => {
  it('configures item and deletes when there is same sku', async () => {
    ...
    expect(contextMock.commit).toHaveBeenNthCalledWith(1, types.CART_DEL_ITEM, {
      product: product2,
    })
    expect(contextMock.commit).toHaveBeenNthCalledWith(2, types.CART_UPD_ITEM_PROPS, {
  ...
  it('configures item', async () => {
    ...
    await (cartActions as any).configureItem(contextMock, { product: product1, configuration: {} })
    expect(contextMock.commit).not.toHaveBeenNthCalledWith(1, types.CART_DEL_ITEM, {
      product: product2,
    })
    expect(contextMock.commit).toHaveBeenNthCalledWith(1, types.CART_UPD_ITEM_PROPS, {
      product: { ...product1, ...product2 },
    })
    expect(contextMock.dispatch).toBeCalledWith('sync', { forceClientState: true })
  })
  ...
  it('adds item to the cart', async () => {
    ...
    expect(contextMock.commit).toHaveBeenNthCalledWith(1, types.CART_ADDING_ITEM, {
      isAdding: true,
    })
    expect(contextMock.dispatch).toBeCalledWith('addItems', {
      productsToAdd: [product1],
      forceServerSilence: false,
    })
    expect(contextMock.commit).toHaveBeenNthCalledWith(2, types.CART_ADDING_ITEM, {
      isAdding: false,
    })
  })
  ...
  it('adds items to the cart', async () => {
    ...
    expect(contextMock.commit).toBeCalledWith(types.CART_ADD_ITEM, {
