...
const invokeClientEntry = async () => {
  const dynamicRuntimeConfig = window.__INITIAL_STATE__.config
    ? Object.assign(globalConfig, window.__INITIAL_STATE__.config)
    : globalConfig
  ...
  function _ssrHydrateSubcomponents (components, next, to) {
    Promise.all(
      ...
      .then(() => {
        AsyncDataLoader.flush({ store, route: to, context: null })
          .then(next)
          .catch(err => {
  ...
  router.onReady(async () => {
    ...
    const canBeMounted = () =>
      RouterManager.isRouteDispatched() && // route is dispatched
    ...
    router.beforeResolve((to, from, next) => {
      ...
      Promise.all(
        matched.map((c: any) => {
          // TODO: update me for mixins support
          const components =
            c.mixins && globalConfig.ssr.executeMixedinAsyncData ? Array.from(c.mixins) : []
          ...
          if (c.asyncData) {
            c.asyncData({ store, route: to })
              .then(result => {
                // always execute the asyncData() from the top most component first
