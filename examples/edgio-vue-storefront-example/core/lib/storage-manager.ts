...
const StorageManager = {
  ...
  init: function (
    ...
    isLocalized = !config.storeViews.commonCache,
    storageQuota = 0
  ) {
    this.storageMap[collectionName] = _prepareCacheStorage(
      collectionName,
      isLocalized,
      storageQuota
    )
  ...
  get: function (collectionName): UniversalStorage {
    if (!this.exists(collectionName)) {
      Logger.warn(
        'Called cache collection ' + collectionName + ' does not exist. Initializing.',
        'cache'
      )
    ...
    const promiseArray = Object.keys(this.storageMap).map(collectionName => {
      return (
        (config.localForage.preserveCollections || []).every(
          collectionToKeep => collectionName !== collectionToKeep
        ) &&
        this.storageMap[collectionName].clear().then(() => {
          Logger.warn(
            `storeManager cleared: ${collectionName}`,
            `storeManager cleared: ${collectionName}`,
            `storeManager cleared: ${collectionName}`
          )()
        })
