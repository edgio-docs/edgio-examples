...
export const defaultCustomOptionValue = (customOption: CustomOption): InputValue => {
  switch (customOption.type) {
    case 'radio': {
      return customOption.values && customOption.values.length
        ? customOption.values[0].option_type_id
        : 0
    }
    case 'checkbox': {
      return []
...
export const selectedCustomOptionValue = (
  optionType: string,
  ...
  switch (optionType) {
    ...
    case 'drop_down': {
      const selectedValue = optionValues.find(
        value => value.option_type_id === (inputValue as number)
      )
    ...
    case 'checkbox': {
      ...
      return (
        optionValues
          .filter(value => checkboxOptionValues.includes(value.option_type_id))
...
export const getCustomOptionValues = (
  selectedCustomOptions: SelectedCustomOption[],
  allCustomOptions: CustomOption[]
): OptionValue[] =>
  selectedCustomOptions
    ...
    .reduce(
      (allCustomOptionValues, customOptionValue) => allCustomOptionValues.concat(customOptionValue),
      []
    ) // merge all values in one array

export const getCustomOptionPriceDelta = (
