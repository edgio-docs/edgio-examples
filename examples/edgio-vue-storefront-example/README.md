...
- The first command will build Vue Storefront app files in `dist` folder
- The second command will build layer0 files in `.layer0`, `dist-layer0-*` folders
- For the next builds it's recommended to run `yarn layer0:clean` before running `yarn layer0:build`

## Run

```
yarn layer0:start:prod
```
...
- If you need to change the HTML template file, go to `/src/themes/default/templates` and modify these files.
  For example, you can see how the scripts required for layer0 have been added there, like this:
