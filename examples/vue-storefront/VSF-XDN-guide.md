# Adding layer0 to Vue Storefront guide
...
<b>Note</b>: layer0 requires Node version >= 12, so before the installation it's recommended to run:
```bash
nvm use 12
```

## 2. Prepare VSF files for layer0
...
module.exports = {
  ...
  server: {
    path: './dist-layer0-server/server.js',
  },
...
```
- Add `layer0/webpack.layer0.client.config.js` file:
```js
...
export default {
  ...
  // devtool: 'source-map',
  output: {
    path: path.resolve(__dirname, '../../dist-layer0-server'),
    publicPath: '/dist-layer0-server/',
    filename: 'server.js',
...
```json
...
"layer0:build:server": "cross-env NODE_ENV=production TS_NODE_PROJECT=\"tsconfig-build.json\" webpack --config ./core/build/webpack.layer0.config.ts --mode production --progress --hide-modules",
"layer0:build:assets": "ncp ./src/themes/default/assets ./dist-layer0-assets",
"layer0:build:client": "cross-env NODE_ENV=production webpack --progress --config layer0/webpack.layer0.client.config.js && layer0 build",
"layer0:build:config": "mkdir ./.layer0/lambda/config && ncp ./config/default.json ./.layer0/lambda/config/production.json",
"layer0:build:ssr-files": "mkdir ./.layer0/lambda/dist && ncp ./dist ./.layer0/lambda/dist",
"layer0:build": "npm run layer0:build:server && npm run layer0:build:assets && npm run layer0:build:client && npm run layer0:build:config && npm run layer0:build:ssr-files",
"layer0:clean": "rimraf dist-layer0-assets && rimraf dist-layer0-client && rimraf dist-layer0-server && rimraf .layer0",
"layer0:start:prod": "layer0 run --production",
"layer0:deploy": "layer0 deploy --team=moovweb-demos --site=vsf1 --skip-build"
...
```
- Ignore layer0 build in `.gitignore`:
```bash
# layer0
/dist-layer0-assets
/dist-layer0-client
...
/.layer0
```
- Find all `*.template.html` files in VSF app and add layer0 scripts into `<head>` there:
```html
  <!-- layer0 -->
  <script src="/service-worker.js" defer></script>
  <script src="/__layer0__/cache-manifest.js" defer></script>
  <script src="/main.js" defer></script>
  <script src="/__layer0__/devtools/install.js" defer></script>
```
