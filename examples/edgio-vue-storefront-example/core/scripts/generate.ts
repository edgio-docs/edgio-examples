...
async function _renderItems (
  ...
  urlToFileNameMapper = (destPath, item) => {
    ...
    if (results.hits && results.hits.hits.length > 0) {
      results.hits.hits.forEach(async item => {
        ...
        try {
          ...
          output = ssr.applyAdvancedOutputProcessing(
            context,
            output,
            templatesCache,
            true,
...
const _cmdGenerateProducts = async cmd => {
  const getProductsPage = (from, size) =>
    search(
      {
        size: size,
        from: from,
...
const _cmdGenerateCategories = async cmd => {
  const getCategoriesPage = (from, size) =>
    search(
      {
        size: size,
        from: from,
...
const _cmdGenerateCms = async cmd => {
  const getCmsPage = (from, size) =>
    search(
      ...
      if (results.hits && results.hits.hits.length > 0) {
        results.hits.hits.map(page => {
          page._source.url_path = `/i/${page._source.identifier}`
        })
      }
      return results
    })
...
program.command('prepare').action(_cmdPrepare)
