...
const addItem = async ({ res: res1 , body: { cartId , item  } , config: { restBuyerFetch , cartCookie , tokenCookie  } ,  })=>{
    ...
    if (!cartId) {
        const { ID , meta  } = await restBuyerFetch('POST', `/orders/Outgoing`, {}).then((response)=>response
        );
        // Set the cart id and token
        cartId = ID;
        token = meta.token;
        // Set the cart and token cookie
        res1.setHeader('Set-Cookie', [
    ...
    if (item.variantId) {
        specs = await restBuyerFetch('GET', `/me/products/${item.productId}/variants/${item.variantId}`, null, {
            token
        }).then((res)=>res.Specs
        );
    }
