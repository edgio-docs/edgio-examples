...
describe('Checkout actions', () => {
  ...
  describe('placeOrder', () => {
    ...
    it('should place order if return code is successful', async () => {
      ...
      expect(mockContext.dispatch).toHaveBeenNthCalledWith(1, 'order/placeOrder', order, {
        root: true,
      })
    ...
    it('should not place order if return code is not successful', async () => {
      ...
      expect(mockContext.dispatch).not.toHaveBeenNthCalledWith(
        3,
        'cart/clear',
        { sync: false },
        { root: true }
    ...
    it('should log thrown error', async () => {
      mockContext.dispatch.mockImplementation(() => {
        throw new Error()
      })
      await (checkoutActions as any).placeOrder(mockContext, { order })
  ...
  it('savePersonalDetails should configure personal details', async () => {
    ...
    expect(mockContext.commit).toHaveBeenCalledWith(
      types.CHECKOUT_SAVE_PERSONAL_DETAILS,
      personalDetails
    )
  })
  ...
  it('saveShippingDetails should configure shipping details', async () => {
    ...
    expect(mockContext.commit).toHaveBeenCalledWith(
      types.CHECKOUT_SAVE_SHIPPING_DETAILS,
      shippingDetails
    )
  })
  ...
  it('load personal, shipping and payment details from cache', async () => {
    ...
    expect(mockContext.commit).toHaveBeenCalledWith(
      types.CHECKOUT_LOAD_PERSONAL_DETAILS,
      personalDetails
    )
    expect(mockContext.commit).toHaveBeenCalledWith(
      types.CHECKOUT_LOAD_SHIPPING_DETAILS,
      shippingDetails
    )
    expect(mockContext.commit).toHaveBeenCalledWith(
      types.CHECKOUT_LOAD_PAYMENT_DETAILS,
      paymentDetails
    )
  })
  ...
  it('addPaymentMethod should configure add payment method', async () => {
    ...
    expect(mockContext.commit).toHaveBeenCalledWith(
      types.CHECKOUT_ADD_PAYMENT_METHOD,
      paymentMethod
    )
  })
  ...
  it('replacePaymentMethods should replace payment method', async () => {
    ...
    expect(mockContext.commit).toHaveBeenCalledWith(
      types.CHECKOUT_SET_PAYMENT_METHODS,
      paymentMethod
    )
  })
  ...
  it('addShippingMethod should add shipping method', async () => {
    ...
    expect(mockContext.commit).toHaveBeenCalledWith(
      types.CHECKOUT_ADD_SHIPPING_METHOD,
      shippingMethod
    )
  })
  ...
  it('replaceShippingMethods should replace shipping method', async () => {
    ...
    expect(mockContext.commit).toHaveBeenCalledWith(
      types.CHECKOUT_SET_SHIPPING_METHODS,
      shippingMethod
    )
  })
})
