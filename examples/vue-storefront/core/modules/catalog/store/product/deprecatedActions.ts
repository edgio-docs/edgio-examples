...
const actions = {
      ...
      await context
        ...
        .then(async categories => {
          ...
          for (let catId of categoryIds) {
            let category = categories.items.find(itm => {
              return toString(itm['id']) === toString(catId)
            })
            if (category) {
          ...
          if (typeof catForBreadcrumbs !== 'undefined') {
            await context
              .dispatch(
                'category/single',
                { key: 'id', value: catForBreadcrumbs.id },
                { root: true }
              )
              .then(() => {
      ...
      if (product.product_links && product.product_links.length > 0) {
        for (let pl of product.product_links) {
          if (pl.link_type === 'associated' && pl.linked_product_type === 'simple') {
            // prefetch links
            Logger.debug(
              'Prefetching grouped product link for ' + pl.sku + ' = ' + pl.linked_product_sku
            )()
            subloaders.push(
              context
                .dispatch('single', {
  ...
  async configure (
    ...
    Logger.warn(
      '`product/configure` deprecated, will be not used from 1.12, use "product/getProductVariant"'
    )()
  ...
  setCurrentOption (context, productOption) {
    ...
    if (productOption && typeof productOption === 'object') {
      ...
      context.commit(
        ...
        Object.assign({}, context.getters.getCurrentProduct, { product_option: productOption })
      )
    }
  },
  ...
  setCurrentErrors (context, errors) {
    ...
    if (errors && typeof errors === 'object') {
      context.commit(
        types.PRODUCT_SET_CURRENT,
        Object.assign({}, context.getters.getCurrentProduct, { errors: errors })
      )
    }
  },
  /**
