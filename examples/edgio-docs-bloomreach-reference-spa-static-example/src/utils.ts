...
export function loadCommerceConfig(): CommerceConfig {
  const channelParams = useContext(
    BrPageContext
  )?.getChannelParameters<ChannelParameters>();
  const commerceConfig: CommerceConfig = {
    graphqlServiceUrl:
      process.env.REACT_APP_GRAPHQL_SERVICE_URL ||
      channelParams?.graphql_baseurl ||
      "http://localhost:4000",
    connector: process.env.REACT_APP_DEFAULT_CONNECTOR ?? "",
    smAccountId:
      channelParams?.smAccountId || process.env.REACT_APP_DEFAULT_SM_ACCOUNT_ID,
    smAuthKey: process.env.REACT_APP_DEFAULT_SM_AUTH_KEY,
    smDomainKey:
      channelParams?.smDomainKey || process.env.REACT_APP_DEFAULT_SM_DOMAIN_KEY,
    ...
    smCustomVarListPriceField:
      process.env.REACT_APP_SM_CUSTOM_VARIANT_LIST_PRICE_FIELD,
    smCustomVarPurchasePriceField:
      process.env.REACT_APP_SM_CUSTOM_VARIANT_PURCHASE_PRICE_FIELD,
  };
