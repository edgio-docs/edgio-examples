...
import { getMediaGallery, configurableChildrenImages, attributeImages } from '.'
...
export default function getProductGallery (product: Product) {
  ...
  const productGallery = uniqBy(
    configurableChildrenImages(product).concat(getMediaGallery(product)),
    'src'
