...
const mergeActions = {
  ...
  async updateServerItem (
    { getters, rootGetters, commit, dispatch },
    { clientItem, serverItem, updateIds, mergeQty }
  ) {
    ...
    if (!rootGetters['checkout/isUserInCheckout']) {
      ...
      diffLog.pushNotification(
        isThisNewItemAddedToTheCart
          ? notifications.productAddedToCart()
          : notifications.productQuantityUpdated()
      )
    }
  ...
  async synchronizeServerItem (
    { dispatch },
    { serverItem, clientItem, forceClientState, dryRun, mergeQty }
  ) {
    const diffLog = createDiffLog()
    ...
    if (!serverItem) {
      ...
      if (forceClientState || !config.cart.serverSyncCanRemoveLocalItems) {
        const updateServerItemDiffLog = await dispatch('updateServerItem', {
          clientItem,
          serverItem,
          updateIds: false
        })
    ...
    if (serverItem.qty !== clientItem.qty || mergeQty) {
      ...
      diffLog.pushServerParty({
        sku: clientItem.sku,
        status: 'wrong-qty',
        'client-qty': clientItem.qty,
        'server-qty': serverItem.qty
      })
      if (dryRun) return diffLog
  ...
  async mergeClientItem (
    { dispatch },
    { clientItem, serverItems, forceClientState, dryRun, mergeQty }
  ) {
    const serverItem = serverItems.find(itm => productsEquals(itm, clientItem))
    const diffLog = await dispatch('synchronizeServerItem', {
      serverItem,
      clientItem,
      forceClientState,
      dryRun,
    ...
    Logger.info(
      'Server and client item with SKU ' + clientItem.sku + ' synced. Updating cart.',
      'cart',
      'cart'
    )()
    if (!dryRun) {
  ...
  async mergeClientItems (
    { dispatch },
    { clientItems, serverItems, forceClientState, dryRun, mergeQty }
  ) {
    const diffLog = createDiffLog()

    for (const clientItem of clientItems) {
      try {
        const mergeClientItemDiffLog = await dispatch('mergeClientItem', {
          clientItem,
          serverItems,
          forceClientState,
          dryRun,
  ...
  async mergeServerItem (
    { dispatch, getters },
    { clientItems, serverItem, forceClientState, dryRun }
  ) {
    const diffLog = createDiffLog()
    ...
    if (forceClientState) {
      ...
      return diffLog.pushServerResponse({
  ...
  async mergeServerItems ({ dispatch }, { serverItems, clientItems, forceClientState, dryRun }) {
    ...
    for (const serverItem of definedServerItems) {
      try {
        const mergeServerItemDiffLog = await dispatch('mergeServerItem', {
          clientItems,
          serverItem,
          forceClientState,
          dryRun
  ...
  async merge (
    { getters, dispatch },
    { serverItems, clientItems, dryRun = false, forceClientState = false, mergeQty = false }
  ) {
    ...
    EventBus.$emit('servercart-after-diff', {
      diffLog: diffLog,
      serverItems: hookResult.serverItem,
      clientItems: hookResult.clientItems,
      dryRun: dryRun
    })
