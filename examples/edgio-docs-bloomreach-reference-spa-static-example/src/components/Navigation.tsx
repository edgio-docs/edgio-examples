...
export function Navigation({
  component,
  page,
}: BrProps): React.ReactElement | null {
  const models = component.getModels<NavigationModels>();
  const params = component.getParameters();
  const documents = useMemo(() => {
    return DOCUMENT_PARAMS.map(
      (model) => models[model as keyof NavigationModels]
    )
  ...
  return (
    <div>
      ...
      <Nav
        as="ul"
        className="justify-content-center justify-content-lg-start mx-n3"
      >
        {documents.map((document) => (
          <Nav.Item key={document.getId()} as="li">
            <Prefetch url={document.getUrl()}>
              <Nav.Link
                as={Link}
