...
describe('Category page', () => {
  it('verification of filters in the Women category', () => {
    ...
    cy.get('[data-testid=categoryButton]').contains('Women').click({ force: true })
    ...
    cy.get('[aria-label="Price < $50"]').first().click().should('have.class', 'active')
    cy.get('[aria-label="Price > $150"]').first().click().should('have.class', 'active')
    cy.get('[data-testid=noProductsInfo]').contains('No products found!')
    cy.get('[aria-label="Price > $150"]').first().click()
