...
describe('Cart mergeActions', () => {
  ...
  it('updates server item - removes when updating was not successful', async () => {
    ...
    ;(CartService.updateItem as jest.Mock).mockImplementation(() =>
      Promise.resolve({ resultCode: 500 })
    )
    ...
    expect(contextMock.commit).toBeCalledWith(types.CART_DEL_ITEM, {
      product: clientItem,
  ...
  it('updates server item - restoring quantity', async () => {
    ...
    expect(contextMock.commit).not.toBeCalledWith(types.CART_DEL_ITEM, {
      product: clientItem,
      removeByParentSku: false,
    })
  ...
  it('updates server item - deletes non confirmed item', async () => {
    ...
    expect(contextMock.commit).toBeCalledWith(types.CART_DEL_NON_CONFIRMED_ITEM, {
      product: clientItem,
    })
  })
  ...
  it('updates server item - apply changes for client item', async () => {
    ...
    ;(createCartItemForUpdate as jest.Mock).mockImplementation(() => clientItem)
    ;(CartService.updateItem as jest.Mock).mockImplementation(() =>
      Promise.resolve({ resultCode: 200, result: serverItem })
    )
    ...
    expect(contextMock.dispatch).not.toBeCalledWith('restoreQuantity', { product: clientItem })
    expect(contextMock.commit).not.toBeCalledWith(types.CART_DEL_NON_CONFIRMED_ITEM, {
      product: clientItem,
    })
    expect(contextMock.dispatch).toBeCalledWith('updateClientItem', {
      clientItem,
  ...
  it('synchronizes item with server when there is no given server item', async () => {
    ...
    expect(contextMock.dispatch).toBeCalledWith('updateServerItem', {
      clientItem,
      serverItem: null,
      updateIds: false,
    })
  ...
  it('synchronizes item with server when there quantities are different', async () => {
    ...
    expect(contextMock.dispatch).not.toBeCalledWith('updateServerItem', {
      clientItem,
      serverItem: null,
      updateIds: false,
    })
  ...
  it('merges client item', async () => {
    ...
    await (cartActions as any).mergeClientItem(contextMock, {
      clientItem,
      serverItems: [serverItem],
      forceClientState: false,
      dryRun: false,
    })
    expect(contextMock.dispatch).toHaveBeenNthCalledWith(1, 'synchronizeServerItem', {
      serverItem,
      clientItem,
      forceClientState: false,
      dryRun: false,
  ...
  it('merges server item', async () => {
    ...
    await (cartActions as any).mergeServerItem(contextMock, {
    ...
    expect(contextMock.dispatch).toHaveBeenNthCalledWith(2, 'addItem', {
      productToAdd: serverItem,
  ...
  it('updates totals after merge', async () => {
    ...
    await (cartActions as any).updateTotalsAfterMerge(contextMock, {
      clientItems: [clientItem],
      dryRun: false,
    })
  ...
  it('merges client and server cart', async () => {
    ...
    await (cartActions as any).merge(contextMock, {
      clientItems: [clientItem],
      serverItems: [serverItem],
    })
    expect(contextMock.dispatch).toHaveBeenNthCalledWith(1, 'mergeClientItems', {
      clientItems: [clientItem],
      serverItems: [serverItem],
      dryRun: false,
      forceClientState: false,
    ...
    expect(contextMock.dispatch).toHaveBeenNthCalledWith(2, 'mergeServerItems', {
      clientItems: [clientItem],
      serverItems: [serverItem],
      dryRun: false,
      forceClientState: false,
    ...
    expect(contextMock.dispatch).toHaveBeenNthCalledWith(3, 'updateTotalsAfterMerge', {
      clientItems: [clientItem],
      dryRun: false,
    })
  })
