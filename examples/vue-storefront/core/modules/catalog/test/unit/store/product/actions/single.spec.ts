...
describe('product/single action', () => {
  ...
  it('should throw error if there is no option value based on key', async () => {
    ...
    await expect(wrapper(productActions)).rejects.toThrow(
      'Please provide the search key sku for product/single action!'
    )
  })
  ...
  it('should set current product', async () => {
    const wrapper = (actions: any) =>
      actions.single(contextMock, { options, setCurrentProduct: true })
