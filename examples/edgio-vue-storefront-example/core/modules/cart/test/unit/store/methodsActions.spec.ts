...
describe('Cart methodsActions', () => {
  it('fetches payment and shipping methods', async () => {
    ...
    expect(contextMock.dispatch).toHaveBeenNthCalledWith(1, 'syncShippingMethods', {
      forceServerSync: false,
    })
    expect(contextMock.dispatch).toHaveBeenNthCalledWith(2, 'syncPaymentMethods', {
      forceServerSync: false,
    })
  })
  ...
  it('synchronizes payment methods', async () => {
    ...
    ;(CartService.getPaymentMethods as jest.Mock).mockImplementation(() =>
      Promise.resolve({ result: {} })
    )
    ;(createOrderData as jest.Mock).mockImplementation(() => ({ shippingMethodsData: {} }))
    ;(preparePaymentMethodsToSync as jest.Mock).mockImplementation(() => ({
      uniqueBackendMethods: [],
      paymentMethods: [],
    }))

    await (cartActions as any).syncPaymentMethods(contextMock, {})
    expect(contextMock.dispatch).toBeCalledWith('checkout/replacePaymentMethods', [], {
      root: true,
    })
  })
  ...
  it('synchronizes shipping methods', async () => {
    ...
    ;(CartService.getShippingMethods as jest.Mock).mockImplementation(() =>
      Promise.resolve({ result: [] })
    )
  ...
  it('updates shipping methods', async () => {
    const contextMock = createContextMock()
    await (cartActions as any).updateShippingMethods(contextMock, {
      shippingMethods: [{ method: 1 }],
    })
    expect(contextMock.dispatch).toBeCalledWith(
      'checkout/replaceShippingMethods',
      [{ is_server_method: true, method: 1 }],
      { root: true }
    )
  })
  ...
  it("doesn't add not available method", async () => {
    const contextMock = createContextMock()
    await (cartActions as any).updateShippingMethods(contextMock, {
      shippingMethods: [{ method: 1, available: false }],
    })
    expect(contextMock.dispatch).toBeCalledWith('checkout/replaceShippingMethods', [], {
      root: true,
    })
  })
})
