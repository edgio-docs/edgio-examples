...
describe('User actions', () => {
  ...
  describe('startSession action', () => {
    ...
    it('should set user token', async () => {
      ...
      expect(contextMock.commit).toBeCalledWith(types.USER_TOKEN_CHANGED, {
  ...
  describe('resetPassword action', () => {
    it('should return response from resetPassword', () => {
      ;(UserService.resetPassword as jest.Mock).mockImplementation(() => data.responseOb)
  ...
  describe('login action', () => {
    it('should return login response', async () => {
      ;(UserService.login as jest.Mock).mockImplementation(async () => data.responseOb)
  ...
  describe('register action', () => {
    it('should return response from register', async () => {
      ;(UserService.register as jest.Mock).mockImplementation(async () => data.responseOb)
  ...
  describe('refresh action', () => {
    it('should update user token', async () => {
      ...
      ;(UserService.refreshToken as jest.Mock).mockImplementation(async () => data.lastUserToken)
  ...
  describe('setUserGroup action', () => {
    it('should update user groupToken and groupId in state', () => {
      ...
      expect(contextMock.commit).toHaveBeenNthCalledWith(
        1,
        types.USER_GROUP_TOKEN_CHANGED,
        data.user.groupToken
      )
      expect(contextMock.commit).toHaveBeenNthCalledWith(
        2,
        types.USER_GROUP_CHANGED,
        data.user.group_id
      )
    })
  })
  ...
  describe('me action', () => {
    ...
    it('should load current user profile if getToken is not empty', async () => {
      ...
      expect(contextMock.dispatch).toHaveBeenNthCalledWith(2, 'refreshUserProfile', {
        resolvedFromCache,
      })
    })
  })
  ...
  describe('update action', () => {
    it('should set current result if resultCode from response is 200', async () => {
      ...
      expect(contextMock.dispatch).toHaveBeenCalledWith('user/setCurrentUser', responseOb.result, {
        root: true,
      })
    })
  })
  ...
  describe('changePassword action', () => {
    it('should call login action if response code is 200', async () => {
      ;(UserService.changePassword as jest.Mock).mockImplementation(async () => data.responseOb)
    ...
    it('should call spawnNotification if response code is not 200', async () => {
      ...
      ;(UserService.changePassword as jest.Mock).mockImplementation(async () => responseOb)
  ...
  describe('clearCurrentUser action', () => {
    it('should clear current user', () => {
      ...
      expect(contextMock.dispatch).toHaveBeenNthCalledWith(1, 'wishlist/clear', null, {
        root: true,
      })
  ...
  describe('loadOrdersFromCache action', () => {
    it('should return ordersHistory', async () => {
      ...
      expect(contextMock.commit).toBeCalledWith(
        types.USER_ORDERS_HISTORY_LOADED,
        data.ordersHistory
      )
  ...
  describe('getOrdersHistory action', () => {
    ...
    it('should dispatch loadOrdersFromCache if useCache is set to true and refreshOrdersHistory action if refresh is set to true', async () => {
      ...
      await (userActions as any).getOrdersHistory(contextMock, {
        refresh,
        useCache,
        pageSize,
        currentPage,
  ...
  describe('sessionAfterAuthorized action', () => {
    it('should call me and getOrdersHistory after authorized', async () => {
      ...
      expect(contextMock.dispatch).toHaveBeenNthCalledWith(2, 'getOrdersHistory', {
        refresh,
        useCache,
      })
    })
