...
import { edgioCommerceSidebar as Sidebar } from '@/components/Sidebar'
    ...
    <div className="flex-col items-center justify-start">
      <div className="flex w-full flex-row items-start px-5">
        ...
        <div className="flex flex-col items-start pt-5">
          ...
          <div className="mt-5 grid grid-cols-1 pb-20 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6">
...
export async function getServerSideProps({ req, query }) {
  ...
  console.log(`${getOrigin(req)}/edgio-api/${query.name ? `categories/${query.name}` : 'products/all'}`, resp)
  if (!resp.ok) {
    return {
      notFound: true,
    }
