...
const Product = ({ data }) => {
  return (
    <div className="flex w-full flex-col items-center">
      <div className="lg:flex max-w-6xl flex-col text-center lg:flex-row lg:pl-10">
        <div className="relative sm:inline-block text-center sm:flex-none sm:w-full md:w-[600px] lg:flex-col lg:w-1/2">
          ...
          <div className="product-thumbnails mt-5 flex flex-row items-start gap-x-2 overflow-x-scroll">
            {data.images.map((i, ind) => (
              <img key={i.url} src={relativizeURL(i.url)} className="h-[150px] w-auto hover:bg-white" />
            ))}
          </div>
        </div>
        <div className="flex w-full text-left flex-col items-start px-10 lg:w-1/2">
