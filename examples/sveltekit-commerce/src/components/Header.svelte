    ...
    <div class="mr-4" class:active={currentRoute === '/'}>
      <a href="/" data-sveltekit-prefetch class="">
        <picture>
          ...
          <img alt="Svelte Logo" class="h-[38] w-[32]" decoding="async" height={38} loading="eager" src="/svelte_logo.png" width={32} />
        </picture>
      </a>
    </div>
      ...
      {#each tabs as tab, i (tab.name)}
        <div class:active={currentRoute === tab.path}>
          <a
            ...
            href={tab.path}
            class={`rounded-lg px-2 py-1 text-white hover:opacity-100 ${currentRoute === tab.path ? 'opacity-100' : 'opacity-75'}`}>{tab.name}</a
          >
        </div>
      {/each}
    </div>
