...
describe('Shipping', () => {
  ...
  beforeEach(() => {
    ...
    mockMethods = Object.entries(Shipping.methods).reduce((result, [methodName]) => {
      result[methodName] = jest
        .spyOn(Shipping.methods, methodName as keyof typeof Shipping.methods)
    ...
    mockHooks = [
      ...
      if (Shipping[hookName]) {
        result[hookName] = jest.spyOn(Shipping, hookName as any).mockImplementation(jest.fn())
      }
  ...
  describe('hooks', () => {
    it('beforeMount hook should start subscription for checkout-after-personalDetails and checkout-after-shippingset events', () => {
      ...
      expect(mockMountingOptions.mocks.$bus.$on).toHaveBeenCalledWith(
        'checkout-after-personalDetails',
        (wrapper.vm as any).onAfterPersonalDetails
      )
      expect(mockMountingOptions.mocks.$bus.$on).toHaveBeenCalledWith(
        'checkout-after-shippingset',
        (wrapper.vm as any).onAfterShippingSet
      )
    })
    ...
    it('beforeDestroy hook should stop subscription for checkout-after-personalDetails and checkout-after-shippingset events', () => {
      ...
      expect(mockMountingOptions.mocks.$bus.$off).toHaveBeenCalledWith(
        'checkout-after-personalDetails',
        (wrapper.vm as any).onAfterPersonalDetails
      )
      expect(mockMountingOptions.mocks.$bus.$off).toHaveBeenCalledWith(
  ...
  describe('methods', () => {
    ...
    it('checkDefaultShippingMethod should configure default shipping method and carrier if current shipping method is falsy', () => {
      ...
      expect((wrapper.vm as any).shipping).toEqual({
        shippingMethod: 'method code 3',
        shippingCarrier: 'carrier code 3',
      })
    })
    ...
    it('onAfterPersonalDetails should configure personal data by dispatching actions', () => {
      ...
      expect(
        mockStore.modules.checkout.actions.updatePropValue
