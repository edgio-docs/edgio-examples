...
describe('Checkout getters', () => {
  it('getShippingDetails should return shipping details with default country from rootState if country is missing in shipping details', () => {
    ...
    const shippingDetails = (checkoutGetters as any).getShippingDetails(
      mockState,
      null,
      mockRootState
    )

    expect(shippingDetails).toEqual({
      ...mockState.shippingDetails,
      country: mockRootState.storeView.tax.defaultCountry,
    })
  })
  ...
  it('isUserInCheckout should inform if user is in checkout if it has been modified less than 30 minutes ago', () => {
    const isUserInCheckout = (checkoutGetters as any).isUserInCheckout({
      modifiedAt: Date.now() - 1000 * 60 * 29,
    })
    const isUserNotInCheckout = (checkoutGetters as any).isUserInCheckout({
      modifiedAt: Date.now() - 1000 * 60 * 31,
    })
  ...
  it('getPaymentMethods should return all configured payment methods if virtual cart is not set', () => {
    ...
    const paymentMethods = (checkoutGetters as any).getPaymentMethods(
      mockState,
      null,
      null,
      rootGetters
  ...
  it('getDefaultPaymentMethod should return default payment method', () => {
    const mockGetters = {
      getPaymentMethods: [
        { code: 'example method 1' },
        { code: 'example method 2', default: true },
        { code: 'cashondelivery' },
      ],
    }
  ...
  it('getNotServerPaymentMethods should return methods not for server', () => {
    const mockGetters = {
      getPaymentMethods: [
        { code: 'example method 1' },
        { code: 'example method 2', is_server_method: true },
      ],
    }
  ...
  it('getShippingMethods should return shipping methods', () => {
    const mockState = {
      shippingMethods: [{ code: 'example method 1' }, { code: 'example method 2' }],
    }
  ...
  it('getDefaultShippingMethod should return default shipping method', () => {
    const mockState = {
      shippingMethods: [{ code: 'example method 1' }, { code: 'example method 2', default: true }],
    }
