...
const showcases = [
  `Edgio's Compatbility with Lit 2.0`,
...
const MyApp = (pageData) => {
  const Component = () => {
    if (window.location.pathname === '/about') {
      const pageProps = { ...pageData, name: 'About', showcases }
      return About(pageProps)
    } else {
      const pageProps = { ...pageData, name: window.location.pathname.substring(1) }
  ...
  return window.html`
    <div class="font-display flex min-h-screen flex-col bg-gradient-to-br from-[#9a1ab1] via-[#004966] to-[#01B18D]">
      <div class="flex flex-row flex-wrap items-center justify-between py-3 px-5">
        ...
        <div class="flex flex-row items-center space-x-4 py-1">
          <a class="text-[#FFFFFF75]" href="/about">
            About
          </a>
          <a href="https://github.com/edgio-docs/edgio-lit-example" target="_blank">
