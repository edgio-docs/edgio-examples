            ...
            {#if data.body.product?.images?.edges.length > 1}
              <div class="absolute right-0 bottom-0 z-40 p-6 ">
                <button
                  ...
                  class="border border-b border-t border-l border-black py-4 px-8"><Icons type="arrowLeft" /></button
                >
                <button
                  on:click={() => {
    ...
    <div class="px-4 py-8">
      ...
      <ul class="grid grid-flow-row grid-cols-2 gap-4 md:grid-cols-4">
        {#each data.body.featuredProducts as product, i (product.node.id)}
          <li>
            <div class="group relative block aspect-square overflow-hidden border border-white/20 bg-zinc-800/50">
              <GridTile removeLabels={true} imageSrc={product.node.images.edges[0].node.originalSrc} href={`/product/${product.node.handle}`} />
            </div>
          </li>
        {/each}
      </ul>
