...
class LocalForageCacheDriver {
  ...
  public recreateDb () {
    if (this._localForageCollection._config) {
      ...
      if (existingConfig.storeName) {
        ...
        const destVersionNumber =
          this._localForageCollection && this._localForageCollection._dbInfo
  ...
  public getItem (key, callback?) {
    ...
    if (!isServer) {
      if (
        this.cacheErrorsCount[this._collectionName] >= DISABLE_PERSISTANCE_AFTER &&
        this._useLocalCacheByDefault
      ) {
        ...
        this._cacheTimeouts.getItem = setTimeout(() => {
          if (!isResolved) {
            ...
            this.cacheErrorsCount[this._collectionName] = this.cacheErrorsCount[
              this._collectionName
            ]
              ? this.cacheErrorsCount[this._collectionName] + 1
              : 1
            const value = this.getLocalCache(key)
            if (isCallbackCallable) callback(null, value)
          }
  ...
  public iterate (iterator, callback?) {
    ...
    this._cacheTimeouts.iterate = setTimeout(() => {
      if (!isResolved) {
        ...
        this.cacheErrorsCount[this._collectionName] = this.cacheErrorsCount[this._collectionName]
          ? this.cacheErrorsCount[this._collectionName] + 1
          : 1
        if (isCallbackCallable) callback(null, null)
      }
  ...
  public setItem (key, value, callback?, memoryOnly = false) {
    ...
    if (!isServer) {
      if (
        this.cacheErrorsCount[this._collectionName] >= DISABLE_PERSISTANCE_AFTER_SAVE &&
        this._useLocalCacheByDefault
      ) {
        if (!this._persistenceErrorNotified) {
