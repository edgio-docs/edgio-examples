...
const actions: ActionTree<CategoryState, RootState> = {
  ...
  single (
    ...
    return new Promise((resolve, reject) => {
      const fetchCat = ({ key, value }) => {
        if (
          key !== 'id' ||
          value >= config.entities.category.categoriesRootCategorylId /* root category */
        ) {
          context
      ...
      if (state.list.length > 0 && !skipCache) {
        ...
        if (
          category &&
          value >= config.entities.category.categoriesRootCategorylId /** root category parent */
        ) {
  ...
  products (
    ...
    if (
      config.entities.twoStageCaching &&
      config.entities.optimize &&
      !isServer &&
      ...
      Logger.log(
        'Using two stage caching for performance optimization - executing first stage product pre-fetching'
      )()
    } else {
      ...
      .then(res => {
        ...
        if (!res || res.noresults) {
          ...
          if (
            ...
            config.products.configurableChildrenStockPrefetchStatic
          ) {
            ...
            res.items.map(i => {
              ...
              if (
                i.type_id === 'configurable' &&
                i.configurable_children &&
                i.configurable_children.length > 0
              ) {
                ...
                if (res.aggregations['agg_terms_' + attrToFilter]) {
                  ...
                  if (res.aggregations['agg_terms_' + attrToFilter + '_options']) {
                    buckets = buckets.concat(
                      res.aggregations['agg_terms_' + attrToFilter + '_options'].buckets
                    )
                  }
    ...
    if (
      ...
      // second stage - request for caching entities; if cacheOnly set - the caching took place with the stage1 request!
      Logger.log(
        'Using two stage caching for performance optimization - executing second stage product caching',
        'category'
