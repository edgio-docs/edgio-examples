...
describe('Order actions', () => {
  ...
  describe('placeOrder action', () => {
    ...
    it('should dispatch processOrder', async () => {
      ...
      expect(contextMock.dispatch).toBeCalledWith('processOrder', {
        newOrder: newOrder,
        currentOrderHash,
      })
    })
    ...
    it('should dispatch processOrder', async () => {
      ;(OrderService.placeOrder as jest.Mock).mockImplementation(async () => task)
      const newOrder: Order = {
        order_id: 'orderId',
        created_at: '10-29-2019',
        updated_at: '11-29-2019',
  ...
  describe('processOrder action', () => {
    it('should add last order with confirmation', async () => {
      ...
      const wrapper = (actions: any) =>
        actions.processOrder(contextMock, { order1, currentOrderHash })
      const processOrderAction = await wrapper(orderActions)

      expect(contextMock.commit).toBeCalledWith(types.ORDER_LAST_ORDER_WITH_CONFIRMATION, {
        order,
    ...
    it('should remove session order hash', async () => {
      ...
      const contextMock = createContextMock()
      const wrapper = (actions: any) =>
        actions.processOrder(contextMock, { order, currentOrderHash })
      const processOrderAction = await wrapper(orderActions)

      expect(contextMock.commit).toBeCalledWith(
        types.ORDER_REMOVE_SESSION_ORDER_HASH,
        currentOrderHash
      )
      expect(processOrderAction).toEqual(task)
    })
  ...
  describe('handlePlacingOrderFailed action', () => {
    it('should dispatch enqueue action', () => {
      ...
      const wrapper = (orderActions: any) =>
        orderActions.handlePlacingOrderFailed(contextMock, { newOrder, currentOrderHash })
