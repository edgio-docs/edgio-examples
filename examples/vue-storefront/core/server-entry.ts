...
function _ssrHydrateSubcomponents (components, store, router, resolve, reject, app, context) {
  Promise.all(
    ...
    .then(() => {
      AsyncDataLoader.flush(
        { store, route: router.currentRoute, context } /* AsyncDataLoaderActionContext */
      )
        .then(r => {
...
export default async context => {
  ...
  const { app, router, store, initialState } = await createApp(
    context,
    context.vs && context.vs.config ? context.vs.config : buildTimeConfig,
    storeCode
  )
      ...
      Promise.all(
        matchedComponents.map((Component: any) => {
          ...
          if (Component.asyncData) {
            Component.asyncData({ store, route: router.currentRoute, context: context })
              .then(result => {
