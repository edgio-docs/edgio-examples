...
describe('Cart totalsActions', () => {
  it('replaces server totals', async () => {
    ...
    await (cartActions as any).overrideServerTotals(contextMock, {
      addressInformation: {},
    ...
    expect(contextMock.commit).toHaveBeenNthCalledWith(1, types.CART_UPD_TOTALS, {
      itemsAfterTotal,
      totals,
  ...
  it('synchronizes totals', async () => {
    ;(createOrderData as jest.Mock).mockImplementation(() => ({
      country: 'US',
      method_code: 'XXX',
    }))
    ...
    expect(contextMock.dispatch).toHaveBeenNthCalledWith(2, 'overrideServerTotals', {
      hasShippingInformation: 'XXX',
