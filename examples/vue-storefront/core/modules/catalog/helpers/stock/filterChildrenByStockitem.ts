...
export default function filterChildrenByStockitem (product, stockItems = []) {
  for (const stockItem of stockItems) {
    ...
    if (
      ...
      (confChild && confChild.status >= 2) /* conf child is disabled */
    ) {
      product.configurable_children = product.configurable_children.filter(
        child => child.id !== stockItem.product_id
      )
    } else {
      if (confChild) {
