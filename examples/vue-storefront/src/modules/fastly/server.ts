...
serverHooks.beforeCacheInvalidated(async ({ tags }) => {
  if (
    !config.get('fastly.enabled') ||
    !config.get('server.useOutputCache') ||
    !config.get('server.useOutputCacheTagging')
  ) {
    return
  }

  console.log('Invalidating Fastly Surrogate-Key')
  const tagsChunks = chunk(
    tags.filter(
      tag =>
    ...
    256
  ) // we can send maximum 256 keys per request, more info https://docs.fastly.com/api/purge#purge_db35b293f8a724717fcf25628d713583

  for (const tagsChunk of tagsChunks) {
    const response = await fetch(
      `https://api.fastly.com/service/${config.get('fastly.serviceId')}/purge`,
      {
        method: 'POST',
