...
function _internalExecute (resolve, reject, task: Task, currentToken, currentCartId) {
  if (currentToken && rootStore.state.userTokenInvalidateLock > 0) {
    // invalidate lock set
    Logger.log(
      'Waiting for rootStore.state.userTokenInvalidateLock to release for ' + task.url,
      'sync'
    )()
    _sleep(1000).then(() => {
      Logger.log(
        'Another try for rootStore.state.userTokenInvalidateLock for ' + task.url,
        'sync'
      )()
    ...
    Logger.error(
      'Aborting the network task' + task.url + rootStore.state.userTokenInvalidateLock,
      'sync'
    )()
    resolve({
      code: 401,
  ...
  if (isCartIdRequired && !currentCartId) {
    // by some reason we does't have the  cart id yet
    reject(
      'Error executing sync task ' +
        task.url +
        ' the required cartId  argument is null. Re-creating shopping cart synchro.'
    )
    return
  }
  ...
  return fetch(url, payload)
    ...
    .then(jsonResponse => {
      if (jsonResponse) {
            ...
            if (config.users.autoRefreshTokens) {
              if (!rootStore.state.userTokenInvalidateLock) {
                ...
                if (
                  ...
                  Logger.info(
                    'Invalidation process in progress (autoRefreshTokens is set to true)' +
                      rootStore.state.userTokenInvalidateAttemptsCount +
                      rootStore.state.userTokenInvalidateLock,
                    'sync'
                  )()
        ...
        Logger.debug(
          'Response for: ' + task.task_id + ' = ' + JSON.stringify(jsonResponse.result),
          'sync'
        )()
        ...
        if (!rootStore.state.userTokenInvalidateLock) {
          // in case we're revalidaing the token - user must wait for it
          resolve(task)
        }
      } else {
...
export function registerSyncTaskProcessor () {
  ...
  EventBus.$on('sync/PROCESS_QUEUE', async data => {
    if (onlineHelper.isOnline) {
      ...
      syncTaskCollection.iterate(
        (task, id) => {
          if (task && !task.transmited && !mutex[id]) {
            // not sent to the server yet
            mutex[id] = true // mark this task as being processed
            fetchQueue.push(
              execute(task, currentUserToken, currentCartToken)
                .then(executedTask => {
