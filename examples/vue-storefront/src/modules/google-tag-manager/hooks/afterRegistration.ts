  ...
  if (isEnabled(config.googleTagManager.id)) {
    ...
    store.subscribe(({ type, payload }, state) => {
      ...
      if (type === 'order/orders/LAST_ORDER_CONFIRMATION') {
        ...
        store.dispatch('user/getOrdersHistory', { refresh: true, useCache: false }).then(() => {
          const orderHistory = state.user.orders_history
          const order = state.user.orders_history
            ? orderHistory.items.find(order => order['entity_id'].toString() === orderId)
            : null
          GTM.trackEvent({
            ecommerce: {
              purchase: {
