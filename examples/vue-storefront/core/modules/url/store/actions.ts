...
export const actions: ActionTree<UrlState, any> = {
      ...
      cacheStorage
        ...
        .then(routeData => {
          if (routeData !== null) {
            ...
            const mappingActionName = config.urlModule.enableMapFallbackUrl
              ? 'mapFallbackUrl'
              : 'mappingFallback'
            dispatch(mappingActionName, { url, params: parsedQuery })
              .then(mappedFallback => {
  ...
  async getFallbackByUrl (context, { url, params }) {
    ...
    try {
      const requestUrl = `${adjustMultistoreApiUrl(
        processURLAddress(config.urlModule.map_endpoint)
      )}`
